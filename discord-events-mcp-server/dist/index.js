...
var EntityType;
(function (EntityType) {
    EntityType[EntityType["STAGE_INSTANCE"] = 1] = "STAGE_INSTANCE";
    EntityType[EntityType["VOICE"] = 2] = "VOICE";
    EntityType[EntityType["EXTERNAL"] = 3] = "EXTERNAL";
})(EntityType || (EntityType = {}));
// Event status
var EventStatus;
(function (EventStatus) {
    EventStatus[EventStatus["SCHEDULED"] = 1] = "SCHEDULED";
    EventStatus[EventStatus["ACTIVE"] = 2] = "ACTIVE";
    EventStatus[EventStatus["COMPLETED"] = 3] = "COMPLETED";
    EventStatus[EventStatus["CANCELED"] = 4] = "CANCELED";
...
- Number of interested users`,
    ...
    try {
        ...
        const events = await discordRequest(`/guilds/${guildId}/scheduled-events${queryParams}`);
        if (events.length === 0) {
            return {
                content: [
                    {
...
server.registerTool("discord_get_event", {
...
times, location, and number of interested users.`,
    ...
    try {
        const queryParams = params.with_user_count ? "?with_user_count=true" : "";
        const event = await discordRequest(`/guilds/${guildId}/scheduled-events/${params.event_id}${queryParams}`);
        return {
            content: [
                {
                    type: "text",
...
server.registerTool("discord_update_event", {
...
Only provide the fields you want to change.`,
    ...
    try {
        const event = await discordRequest(`/guilds/${guildId}/scheduled-events/${params.event_id}`, "PATCH", body);
        return {
            content: [
                {
                    type: "text",
...
This action is irreversible. The event will be permanently removed.`,
    ...
    try {
        await discordRequest(`/guilds/${guildId}/scheduled-events/${params.event_id}`, "DELETE");
        return {
            content: [
                {
                    type: "text",
...
async function main() {
    if (!BOT_TOKEN) {
        console.error("WARNING: DISCORD_BOT_TOKEN environment variable is not set.\n" +
            "The server will start but API calls will fail.\n" +
            "Create a bot at https://discord.com/developers/applications");
    }
    if (!DEFAULT_GUILD_ID) {
        console.error("NOTE: DISCORD_GUILD_ID environment variable is not set.\n" +
